...
def test_wait_idle_rejects_previous_idle_snapshot_until_target_arrives(
    controller: GrblController,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    target = Position(x_mm=-100.0, y_mm=-120.0, z_mm=-74.0)
    previous = controller._status
    arrived = MachineStatus(state=MachineState.IDLE, position=target)
    controller.get_status = MagicMock(side_effect=[previous, arrived])
    monkeypatch.setattr(
        "src.hardware.drivers.gantry.grbl_controller.time.sleep", lambda _: None
    )

    GrblController._wait_idle(controller, timeout_s=1.0, target=target)

    assert controller.get_status.call_count == 2
...
def test_sync_status_poll_preserves_alarm_code_and_limit_pins(
    controller: GrblController,
) -> None:
    controller._ser.readline.side_effect = [
        b"ALARM:1\n",
        b"<Alarm|MPos:-270.000,-5.000,-30.000|Pn:Y>\n",
    ]

    refreshed = GrblController._poll_status_sync(controller, timeout_s=0.5)

    assert refreshed is True
    assert controller._status.state == MachineState.ALARM
    assert controller._status.alarm_code == 1
    assert controller._status.limit_pins == ["Y"]


def test_motion_alarm_reports_pin_position_and_target(
    controller: GrblController,
) -> None:
    controller._status = MachineStatus(
        state=MachineState.ALARM,
        position=Position(x_mm=-270.0, y_mm=-5.0, z_mm=-30.0),
        alarm_code=1,
        limit_pins=["Y"],
        raw="<Alarm|MPos:-270.000,-5.000,-30.000|Pn:Y>",
    )
    controller.get_status = MagicMock(return_value=controller._status)

    with pytest.raises(AlarmStateError) as exc_info:
        GrblController._wait_idle(
            controller,
            timeout_s=0.5,
            target=Position(x_mm=-148.0, y_mm=-172.5, z_mm=-30.0),
        )

    message = str(exc_info.value)
    assert "alarm_code=1" in message
    assert "Pn=Y" in message
    assert "当前位置=(-270.000, -5.000, -30.000)" in message
    assert "目标=(-148.000, -172.500, -30.000)" in message
...
def test_driver_disconnect_does_not_close_shared_relay(
    controller: GrblController,
) -> None:
