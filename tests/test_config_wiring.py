...
hardware:
  ...
  spincoater:
    ...
    max_rpm: 3000
    max_rpm_confirmation_required: true
...
def test_load_config_validates_unified_hardware_schema(tmp_path: Path) -> None:
    ...
    assert cfg.devices["gripper"].backend == "relay"
...
def test_production_hardware_yaml_loads_gantry() -> None:
    ...
    assert cfg.devices is not None
    assert cfg.devices["gantry"].backend == "verified_driver"
    assert cfg.devices["gantry"].driver == "grbl_controller"
    assert gantry.port == "/dev/autospin_xyz"
    assert gantry.feed_rate_default == 1000
    assert gantry.boundary_escape_mm == 10
...
def test_devices_yaml_loads_unverified_gantry_metadata() -> None:
    ...
    assert gantry.migration_status == "code_migrated_hardware_unverified"


def test_devices_yaml_loads_unverified_gripper_metadata() -> None:
    metadata = load_device_metadata()
    ...
    assert gripper.migration_status == "code_migrated_hardware_unverified"
...
def test_rejects_conflicting_gantry_limits(tmp_path: Path) -> None:
    text = CONFIG_TEXT.replace("x_min_mm: -310.0", "x_min_mm: -309.0")
    with pytest.raises(ValueError, match="verified limits"):
        load_config(_write_config(tmp_path, text))


def test_rejects_undeclared_duplicate_relay_channel(tmp_path: Path) -> None:
...
def test_rejects_missing_required_serial_parameter(tmp_path: Path) -> None:
    text = CONFIG_TEXT.replace("    parity: N\n", "", 1)
    with pytest.raises(ValueError, match="parity"):
        load_config(_write_config(tmp_path, text))


def test_rejects_unsafe_placeholder_serial_path(tmp_path: Path) -> None:
    text = CONFIG_TEXT.replace("/dev/test-gantry", "TODO_GANTRY_PORT")
    with pytest.raises(ValueError, match="placeholder"):
        load_config(_write_config(tmp_path, text))


def test_rejects_inverted_gripper_polarity(tmp_path: Path) -> None:
    text = CONFIG_TEXT.replace("    close_level: true", "    close_level: false")
    with pytest.raises(ValueError, match="close_level"):
        load_config(_write_config(tmp_path, text))


def test_config_validation_does_not_access_hardware(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def forbidden_serial_access(*args: object, **kwargs: object) -> None:
        raise AssertionError("configuration validation attempted hardware access")

    monkeypatch.setattr("serial.Serial", forbidden_serial_access)
    cfg = load_config(_write_config(tmp_path))

    assert cfg.hardware is not None
    assert cfg.hardware.gantry is not None
    assert cfg.hardware.gantry.port == "/dev/test-gantry"
