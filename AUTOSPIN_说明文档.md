...
当前完整实验入口是 `autospin` Web 服务的语义实验执行接口：

```text
recipes/*.json + POST /api/experiments/multi-round/execute
```

配方生成、protocol 校验、多轮调度和真实执行都在当前项目中完成，不再运行同级
`AutoSpinmotorSystem` 的脚本。

推荐启动顺序见 `EXPERIMENT_RUNBOOK.md` 的“autospin recipe 启动流程”：
...
在网页实验构建器中保存配方，先运行 Dry-run，通过后再正式运行。真实运行前必须完成
坐标、枪头、样品、真空、夹爪、热台、旋涂台和急停条件检查。
