...
# Status reports may differ by a few encoder/step rounding units at an edge.
# Values beyond this tolerance are not physical positions for this machine.
# In particular, grbl-Mega can expose an int32 overflow sentinel as
# ±2147483.647 mm; accepting that value poisons distance calculations and UI.
_STATUS_POSITION_TOLERANCE_MM = 1.0
...
class GrblController:
    """Verified AutoSpinmotorSystem hardware controller for grbl-Mega-5X.
    ...
    def connect(self) -> None:
        try:
            with self._resource_manager.transaction(
                self._resource_handle, "gantry.connect"
            ):
    ...
    def disconnect(self) -> None:
        """Close the GRBL and brake-controller connections."""
        self.close()
    ...
    def get_status(self) -> MachineStatus:
        ...
        return self._status.model_copy(
            update={
                ...
                # Millisecond epoch floats lose sub-microsecond precision;
                # normalize the diagnostic so a 100 ms-old snapshot never
                # reports as 99.999755 ms.
                "last_update_ms_ago": round(
                    _now_ms() - self._status_ts_ms, 3
                ),
            }
        )
    ...
    def home(
        self, *, idempotency_key: str | None = None, dry_run: bool = False
    ) -> HomeResult | HomePlan:
        ...
        if idempotency_key is None:
            idempotency_key = f"gantry-home-{uuid.uuid4().hex}"
        if dry_run:
            return HomePlan(
                sequence=[
        ...
        **为什么用 `$J=` 不用 `G1`**（Slice 5 回归 ADR-002 §15-18 路线）：
        ...
        if isinstance(target, Position):
            ...
            if y is None or z is None:
                raise TypeError("move_to(x, y, z, feed) requires y and z")
            position = Position(x_mm=float(target), y_mm=float(y), z_mm=float(z))
            if feed is not None:
                if feed_mm_min is not None:
                    raise TypeError("provide either feed or feed_mm_min, not both")
        ...
        # Issue #025 brake-skip must use a fresh machine position.  Using the
        # cached snapshot here can mistake an XY-only move for a Z move after
        # another device was connected or the status poller was delayed.  The
        # unnecessary release/lock cycle is both misleading and an avoidable
        # EMI/mechanical-wear event.
        try:
            refreshed = self._poll_status_sync(timeout_s=0.5)
        except (serial.SerialException, OSError) as exc:
            self._drop_serial()
            raise L3ConnectionError(
                human_message=f"移动前读取 GRBL 位置失败：{exc}",
                agent_message=(
                    "Failed to obtain a fresh GRBL position before deciding "
                    f"whether to release the Z brake: {exc!r}"
                ),
            ) from exc
        if not refreshed:
            raise L3ConnectionError(
                human_message="移动前未收到 GRBL 位置状态，已取消移动",
                agent_message=(
                    "No fresh GRBL status frame before motion; motion was "
                    "cancelled so the Z-brake decision cannot use stale position data."
                ),
            )
        current_status = self.get_status()
        if not current_status.position_valid:
            raise L3Error(
                human_message="GRBL 当前位置无效，已取消移动",
                agent_message=current_status.status_error or "Invalid GRBL MPos.",
            )
        z_unchanged = (
            abs(target.z_mm - current_status.position.z_mm)
            < _Z_UNCHANGED_EPSILON_MM
        )
        ...
        try:
            ...
            try:
                ...
                pass
            if wait_for_idle:
                self._wait_idle(timeout_s=timeout, target=target)
        finally:
            if not z_unchanged:
                try:
                    self._lock_brake()
    ...
    def stop(self) -> MachineStatus:
        """Stop an active jog while preserving homing when GRBL permits."""
        return self.halt()
    ...
    def emergency_stop(self) -> MachineStatus:
        """Immediately reset GRBL and lock the Z brake."""
        return self.abort_motion_immediate()
    ...
    def unlock(self) -> MachineStatus:
        """Clear the current GRBL alarm with ``$X``."""
        return self.unlock_alarm()
            ...
            try:
                ...
                while time.time() < deadline:
                    ...
                    if raw.startswith("ALARM:"):
                        # Do not raise out of the background thread; retain
                        # the alarm code for the following status snapshot.
                        self._make_alarm_error(raw, "background status poll")
                        continue
                    # Other content (ok/error) is discarded here. The active
                    # command path owns the lock while waiting for its ack.
            finally:
                self._lock.release()

    def _poll_status_sync(self, timeout_s: float) -> bool:
        ...
        with self._lock:
            ...
            pending_alarm: AlarmStateError | None = None
            while time.time() < deadline:
                self._ser.write(b"?")
                self._ser.flush()
                ...
                while time.time() < inner_deadline:
                    ...
                    if raw.startswith("<"):
                        ...
                        return True
                    if raw.startswith("ALARM:"):
                        # Preserve the asynchronous alarm number, then keep
                        # reading for <Alarm|...|Pn:...> so diagnostics also
                        # contain the asserted limit pins and last position.
                        pending_alarm = self._make_alarm_error(raw, "status poll")
                        continue
                    if raw.startswith("error:"):
                        raise self._make_alarm_error(raw, "status poll")
    ...
    def _wait_idle(
        ...
        *,
        ...
        status_loss_started: float | None = None
        last_status: MachineStatus | None = None
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            try:
                refreshed = self._poll_status_sync(timeout_s=0.3)
            except (serial.SerialException, OSError) as exc:
                self._drop_serial()
                raise L3ConnectionError(
                    human_message=f"运动期间读取 GRBL 状态失败：{exc}",
                    agent_message=f"Serial status poll failed during motion: {exc!r}",
                ) from exc
            st = self.get_status()
            last_status = st
            if not refreshed or st.last_update_ms_ago > stale_threshold_ms:
                if status_loss_started is None:
                    status_loss_started = time.monotonic()
                if time.monotonic() - status_loss_started >= 2.0:
                    raise L3ConnectionError(
                        human_message=(
                            "运动期间连续 2 秒未收到 GRBL 状态，请检查导轨串口"
                        ),
                        agent_message=(
                            "No fresh GRBL status frame for 2 seconds while "
                            "waiting for Idle; another process may own the port "
                            "or the USB serial link may be interrupted."
                        ),
                    )
                continue
            status_loss_started = None
            if not st.position_valid:
                raise L3Error(
                    human_message=(
                        "龙门控制器返回了无效坐标，运动状态不可确认。"
                        "请立即停止、检查限位输入和接线干扰，然后重新归零。"
                    ),
                    agent_message=st.status_error or "Invalid GRBL MPos.",
                )
            if st.state == MachineState.IDLE:
                # ``ok`` only means GRBL accepted the jog.  Immediately after
                # it, the cached snapshot can still be the Idle report from
                # the previous command.  Do not release the Z brake or let a
                # following actuator move until a reported position actually
                # reaches this command's target.
                if target is None or all(
                    abs(actual - expected) <= _STATUS_POSITION_TOLERANCE_MM
                    for actual, expected in (
                        (st.position.x_mm, target.x_mm),
                        (st.position.y_mm, target.y_mm),
                        (st.position.z_mm, target.z_mm),
                    )
                ):
                    return
            ...
            if st.state == MachineState.ALARM:
                pins = "".join(st.limit_pins) or "none"
                target_text = (
                    "unknown"
                    if target is None
                    else (
                        f"({target.x_mm:.3f}, {target.y_mm:.3f}, "
                        f"{target.z_mm:.3f})"
                    )
                )
                raise AlarmStateError(
                    human_message=(
                        "运动中进 alarm："
                        f"alarm_code={st.alarm_code}，Pn={pins}，"
                        f"当前位置=({st.position.x_mm:.3f}, "
                        f"{st.position.y_mm:.3f}, {st.position.z_mm:.3f})，"
                        f"目标={target_text}"
                    ),
                    agent_message=(
                        f"Post-move state=ALARM:{st.alarm_code}; "
                        f"limit_pins={st.limit_pins}; raw={st.raw!r}; "
                        f"position={st.position}; target={target}; "
                        "inspect hard-limit wiring/EMI, then home to recover."
                    ),
                )
            time.sleep(0.05)
        if (
            last_status is not None
            and last_status.state == MachineState.IDLE
            and target is not None
        ):
            actual = last_status.position
            raise HomingTimeoutError(
                human_message=(
                    "GRBL 已空闲，但实际坐标未到达目标；"
                    "请检查驱动使能、脉冲参数与 Z 刹车线路"
                ),
                agent_message=(
                    "GRBL reported Idle but the target was not reached. "
                    f"target=({target.x_mm:.3f}, {target.y_mm:.3f}, {target.z_mm:.3f}), "
                    f"actual=({actual.x_mm:.3f}, {actual.y_mm:.3f}, {actual.z_mm:.3f}). "
                    "Check $0/$4 and driver enable/pulse wiring; for Z also check "
                    "continuity from DSTUR CH2 through the brake coil."
                ),
            )
        raise HomingTimeoutError(
    ...
    def _parse_status_line(self, raw: str) -> None:
        ...
        limits = self.config.soft_limits
        tolerance = _STATUS_POSITION_TOLERANCE_MM
        invalid_axes = [
            axis
            for axis, value, minimum, maximum in (
                ("X", position.x_mm, limits.x_min_mm, limits.x_max_mm),
                ("Y", position.y_mm, limits.y_min_mm, limits.y_max_mm),
                ("Z", position.z_mm, limits.z_min_mm, limits.z_max_mm),
            )
            if not minimum - tolerance <= value <= maximum + tolerance
        ]
        if self._is_homed and invalid_axes:
            error = (
                "Rejected GRBL MPos outside configured envelope: "
                f"axes={','.join(invalid_axes)}, "
                f"position=({position.x_mm}, {position.y_mm}, {position.z_mm}), "
                f"raw={raw!r}"
            )
            # Do not publish the corrupt coordinates. Preserve the last known
            # trustworthy display value, invalidate homing, and force all
            # subsequent motion paths to stop until a real re-home succeeds.
            self._is_homed = False
            self._status = MachineStatus(
                state=MachineState.UNKNOWN,
