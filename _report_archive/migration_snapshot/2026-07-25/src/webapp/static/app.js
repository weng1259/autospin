...
async function loadProcessCoordinates() {
  try {
    const coordinates = await api("/api/config/process-coordinates");
    document.querySelector('#coordinate-form textarea[name="coordinates"]').value =
      JSON.stringify(coordinates, null, 2);
    setBuilderMessage("工艺坐标已从后端读取");
  } catch (error) {
    setBuilderMessage(`读取失败：${error.message}`, true);
  }
}

function bindBuilderControls() {
  const multiRoundForm = document.querySelector("#multi-round-form");
  multiRoundForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    if (!multiRoundForm.reportValidity()) {
      return;
    }
    const form = new FormData(multiRoundForm);
    try {
      const result = await api("/api/routines/generate-multi-round", {
        source_name: form.get("source_name"),
        output_name: form.get("output_name"),
        rounds: Number(form.get("rounds")),
        offset_x_mm: Number(form.get("offset_x_mm")),
        offset_y_mm: Number(form.get("offset_y_mm")),
        offset_z_mm: Number(form.get("offset_z_mm")),
      });
      setBuilderMessage(`已生成 ${result.name}，共 ${result.step_count} 步`);
      void refreshRoutineList();
    } catch (error) {
      setBuilderMessage(`生成失败：${error.message}`, true);
    }
  });
  ...
  coordinateForm.addEventListener("submit", async (event) => {
    ...
    try {
      const payload = JSON.parse(
        coordinateForm.elements.namedItem("coordinates").value,
      );
      await api("/api/config/process-coordinates", payload, { method: "PUT" });
      setBuilderMessage("工艺坐标已校验并保存");
    } catch (error) {
      setBuilderMessage(`保存失败：${error.message}`, true);
    }
  });
  document.querySelector("#coordinate-load-button").addEventListener(
    "click",
    () => void loadProcessCoordinates(),
  );
}
