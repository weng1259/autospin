...
class EmmLinearStage:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_position_mm = float(travel_mm) if max_position_mm is None else float(max_position_mm)
    ...
    def move_relative(
        ...
        *,
        ...
        allowed_span = self.max_position_mm - self.min_position_mm
        if abs(distance) > allowed_span:
            raise ValueError(f"absolute move distance must be <= {allowed_span} mm")
        if not 1 <= speed <= 3000:
        ...
        if not self.min_position_mm <= target <= self.max_position_mm:
            raise ValueError(
                f"commanded target {target:.3f} mm is outside "
                f"{self.min_position_mm:.3f}..{self.max_position_mm:.3f} mm; "
    ...
    def move_absolute(
        ...
        *,
        ...
        if not self.min_position_mm <= target <= self.max_position_mm:
            raise ValueError(
                f"absolute target {target:.3f} mm is outside "
                f"{self.min_position_mm:.3f}..{self.max_position_mm:.3f} mm"
            )
